...
use crate::syntax::{
    is_at_identifier, parse_declaration_list_block, parse_regular_identifier, parse_string, BODY_RECOVERY_SET,
};
...
#[inline]
pub(crate) fn parse_keyframes_block(p: &mut CssParser) -> ParsedSyntax {
    if !p.at(T!['{']) {
        return Absent;
    }
...
pub(crate) struct CssKeyframesItemList;
...
impl ParseNodeList for CssKeyframesItemList {
    ...
    fn recover(
        ...
        parsed_element.or_recover(
            p,
            &ParseRecovery::new(CSS_BOGUS, KEYFRAMES_ITEM_LIST_RECOVERY_SET),
...
#[inline]
pub(crate) fn parse_keyframes_item(p: &mut CssParser) -> ParsedSyntax {
    ...
    parse_declaration_list_block(p).ok(); // TODO handle error
...
pub(crate) struct CssKeyframesSelectorList;
...
impl ParseSeparatedList for CssKeyframesSelectorList {
    ...
    fn recover(
        ...
        parsed_element.or_recover(
            p,
            &ParseRecovery::new(CSS_BOGUS, KEYFRAMES_ITEM_SELECTOR_LIST_RECOVERY_SET),
...
const KEYFRAMES_ITEM_SELECTOR_IDENT_SET: TokenSet<CssSyntaxKind> =
    token_set!(T![from], T![to], CSS_NUMBER_LITERAL);
...
#[inline]
pub(crate) fn parse_keyframes_item_selector(p: &mut CssParser) -> ParsedSyntax {
    ...
    let kind = if is_at_percentage_dimension(p) {
        ...
        p.bump_ts(KEYFRAMES_ITEM_SELECTOR_SET);
